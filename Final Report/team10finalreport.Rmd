...
This is the very first model that we used as a base case. We also ran other basic linear regression models (LASSO, Ridge Regression, etc.) but they all seemed to perform similarly. The issue was they were unable to fit the trend.
...
From EDA and the linear regression models, we know we need to be able to model change over time or time series. One of the best models for dealing with Time Series is ARIMA and its derivatives. 
