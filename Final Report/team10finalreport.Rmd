...
then we ran join_data.py (which imports from noaa.py) to join the data with weather data from the NOAA API. We used the same process to add the additional weather columns to the 2011 and 2012 as well.
...
Our R code from our EDA applies a season variable to the dataset, based on the date.
...
lines(pred, col = 'red')
```
A naive linear regression does not incorporate any time series factors, has an adjusted R-squared of 0.45
