...
#print(cat("LightGBM RMSE Daily forecast:", lgb_rmse, "\n"))
...
ggplot(df12, aes(x = Date)) +
  ...
  labs(title = paste0("Acutal vs predicted - Day wise forecast Lightgbm RMSE: ",lgb_rmse ),
       x = "Date",
       y = "Values") +
...
# Plotting using ggplot2
ggplot(df12_week, aes(x = Date)) +
  geom_line(aes(y = Actual, color = "Actual"), size = 1) +
  geom_line(aes(y = Predited, color = "Predited"), size = 1) +
  labs(title = paste0("Acutal vs predicted - Week wise forecast Lightgbm RMSE: ", lgb_rmse_week),
       x = "Date",
       y = "Values") +
  scale_color_manual(values = c("Actual" = "blue", "Predited" = "red")) +
  theme_minimal()
